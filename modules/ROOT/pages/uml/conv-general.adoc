...
The below images demonstrate tags set for example UML elements from each of the above groups. <<fig:element-tag>> shows a tag for UML class, <<fig:attribute-tag>> for UML attribute and <<fig:dependency-tag>> for UML dependency connector.
