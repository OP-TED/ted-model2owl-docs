...
* `generate-respec` – Creates a ReSpec HTML documentation package for an ontology using Jinja2 with templates based on XMI file, ReSpec JSON file and static assets. This command does not generate other RDF artefacts, so they need to be generated separately and provided as input to this command.
  ** **Parameters:**
...
7. `make generate-respec` – Generates a ReSpec documentation package.
   * Generated folder:
