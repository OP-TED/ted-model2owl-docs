...
[#rule:association-uni-multiplicity-rc,source,XML,caption='',title='{example-caption} {counter:rule-cnt:2.1}. Association & dependency multiplicity -- in reasoning layer',reftext='{example-caption} {rule-cnt}']
====
...
|===
|
.Min cardinality restriction for association in Turtle syntax
[source,Turtle]
----
:ClassName
...
----
|
.Min cardinality restriction for association in RDF/XML syntax
[source,XML]
----
...
----
|
.Min cardinality restriction for dependency in Turtle syntax
[source,Turtle]
----
:ClassName
  rdfs:subClassOf [ a owl:Restriction ;
...
----
|
.Min cardinality restriction for dependency in RDF/XML syntax
[source,XML]
----
<rdf:Description rdf:about = "http://base.onto.uri/ClassName">
    <rdfs:subClassOf>
...
[#rule:association-uni-multiplicity-one-rc,source,XML,caption='',title='{example-caption} {counter:rule-cnt:2.1}. Association & dependency multiplicity "one" -- in reasoning layer',reftext='{example-caption} {rule-cnt}']
====
...
[#rule:association-uni-multiplicity-ds,source,XML,caption='',title='{example-caption} {counter:rule-cnt:2.1}. Association & dependency multiplicity -- in data shape layer',reftext='{example-caption} {rule-cnt}']
====
...
====
Specify a default container type for each UML association/dependency that can accept more than a one value by setting the fixed `@set` keyword as a value for the `@container` key.
...
====
Specify a sub-property axiom representing the generalisation between named roles of UML associations. Properties on the subject and object positions correspond to the association roles belonging to the same side of the association ends. 
Such a sub-property axiom is specified for the target side of association ends (for unidirectional associations) or for both source and target sides of the association ends (for bidirectional associations).
====
